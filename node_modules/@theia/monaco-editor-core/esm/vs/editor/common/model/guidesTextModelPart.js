...
class GuidesTextModelPart extends textModelPart_1.TextModelPart {
    ...
    getLinesBracketGuides(startLineNumber, endLineNumber, activePosition, options) {
        ...
        if (activePosition && bracketPairs.length > 0) {
            const bracketsContainingActivePosition = startLineNumber <= activePosition.lineNumber &&
                ...
                ? // Does active position intersect with the view port? -> Intersect bracket pairs with activePosition
                    bracketPairs.filter((bp) => range_1.Range.strictContainsPosition(bp.range, activePosition))
                : this.textModel.bracketPairs.getBracketPairsInRange(range_1.Range.fromPositions(activePosition));
            activeBracketPairRange = (_a = (0, arrays_1.findLast)(bracketsContainingActivePosition, 
        ...
        const queue = new arrays_1.ArrayQueue(bracketPairs);
            ...
            for (const line of activeGuides) {
                ...
                if ((isActive &&
                    ...
                    (options.includeInactive &&
                        options.horizontalGuides === textModelGuides_1.HorizontalGuidesState.Enabled)) {
                    if (line.start.lineNumber === lineNumber) {
                        if (line.guideVisibleColumn < line.visibleStartColumn) {
                            guides.push(new textModelGuides_1.IndentGuide(line.guideVisibleColumn, className, new textModelGuides_1.IndentGuideHorizontalLine(false, line.start.column)));
                        }
                    }
                    if (line.end.lineNumber === lineNumber + 1) {
                        ...
                        if (line.guideVisibleColumn < line.visibleEndColumn) {
                            nextGuides.push(new textModelGuides_1.IndentGuide(line.guideVisibleColumn, className, new textModelGuides_1.IndentGuideHorizontalLine(!line.renderHorizontalEndLineAtTheBottom, line.end.column)));
                        }
                    }
                }
            }
            ...
            for (let i = activeGuides.length - 1; i >= 0; i--) {
                ...
                if (isActive || options.includeInactive) {
                    if (line.renderHorizontalEndLineAtTheBottom &&
                        ...
                        nextGuides.push(new textModelGuides_1.IndentGuide(line.guideVisibleColumn, className, null));
                    }
                }
                ...
                if (isActive || options.includeInactive) {
                    guides.push(new textModelGuides_1.IndentGuide(line.guideVisibleColumn, className, null));
                }
            }
